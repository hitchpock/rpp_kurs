...
def add_student():
    ...
    if form1.validate_on_submit() and form1.submit.data:
        ...
        old_student = Student.query.filter(Student.u_id == form1.u_id.data) \
                                   .first()
        ...
        # db.session.add(user)
        student = Student(name=form1.name.data, std_id=user.id,
                          grp_name=form1.grp_name.data, u_id=form1.u_id.data)
    ...
    student_list = [(i.std_id, i.name + "\t" + i.grp_name)
                    for i in db_student_list]
...
def put_mark():
    if not current_user.is_authenticated or current_user.role != 'teacher':
        return redirect(url_for('login'))
    db_student_list = Student.query.all()
    student_list = [(i.u_id, i.grp_name + "\t" + i.name)
                    for i in db_student_list]
    form = PutMark()
    form.student.choices = student_list
    if form.validate_on_submit():
        teacher = Teacher.query.filter(Teacher.teach_id == current_user.id) \
                               .first()
...
def check_mark():
    ...
    session_lst = session_number(Session.query.filter(
                                 Session.u_id == student.u_id).all())
